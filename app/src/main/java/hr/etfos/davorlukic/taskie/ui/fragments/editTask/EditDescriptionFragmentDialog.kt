...
class EditDescriptionFragmentDialog: DialogFragment() {
    ...
    private fun initListeners() {
        saveEditAction.setOnClickListener {
            repository.editTaskDescription(task.id, editTaskInput.text.toString())
    ...
    private fun initUi() {
        editTaskHeading.text = getString(R.string.editTaskDescriptionHeadingText)
        editTaskInput.setText(task.description)
    }

    companion object{
        fun newInstance(): EditDescriptionFragmentDialog {
            return EditDescriptionFragmentDialog()
        }
    }
}